...
//        CheckGLError();
     ...
     * corresponds to function AspectRatio.cpp::reshape().
     *
    ...
    virtual void framebufferResizeEvent(glowwindow::ResizeEvent& event) override {
        ...
        //retrieve the perspective matrix stored in the uniform object
        glm::mat4 matrix = perspectiveMatrix->value();
        
        /*
         * Set the x-component of the 1st column (0th column in C & co. style programing parlance)
         * according to the new window size. Actually, the new frame buffer size but let's ignore
         * this for a moment ;).
         */
        matrix[0].x = FRUSTUM_SCALE / (static_cast<float>(event.width()) / static_cast<float>(event.height()));
        ...
        /*
         * Resizes the viewport according to the new dimension provided by the event object.
         */
...
//        CheckGLError();
         ...
         */
        theProgram->setUniform(offsetUniform->name(), glm::vec2 {1.5f, 0.5f});
//        glUniform2f(theProgram->getUniformLocation(offsetUniform->name()), 1.5f, 0.25f);
//        CheckGLError();
...
protected:
    ...
    void initializeProgram () {
        ...
        float fFrustumScale {FRUSTUM_SCALE};
        float fzNear {0.5f};
        float fzFar {3.0f};
...
int main(int /*argc*/, char* /*argv*/[]) {
    ...
    if (window.create(format, "Learning Modern 3D Graphics Programming [with glow] -- Aspect Ratio")) {
