...
protected:
    ...
    void initializeProgram () {
		...
		theProgram->attach(
                           glowutils::createShaderFromFile(GL_VERTEX_SHADER, "data/arcsynthesis/chapter4/arbitrary-perspective-plane-further-studies.vert"),
                           glowutils::createShaderFromFile(GL_FRAGMENT_SHADER, "data/arcsynthesis/chapter4/arbitrary-perspective-plane-further-studies.frag")
                           );
        
        // create the uniforms used in the vertex shader
        offsetUniform = theProgram->getUniform<glm::vec2>("offset");
        eyeUniform = theProgram->getUniform<glm::vec4>("eye");
        ...
        theProgram->use();
        // set the eyeUniform that defines the position of the eye/camera relative to
        // the center of the projection plane. assuming that the eye is at the origin
        // (0.0f, 0.0f, 0.0f)
        theProgram->setUniform(eyeUniform->name(), glm::vec4 {0.f, 0.f, -1.0f, 0.f});
...
private:
    ...
    /**
     * The uniform that maintains the eye position.
     */
    glow::Uniform<glm::vec4>* eyeUniform;
