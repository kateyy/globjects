...
void WeightedAverageAlgorithm::initialize(const std::string & transparencyShaderFilePath, glow::Shader *vertexShader, glow::Shader *geometryShader) {
	...
	m_opaqueProgram->attach(glow::Shader::fromFile(gl::FRAGMENT_SHADER, transparencyShaderFilePath + "wavg_opaque.frag"));
	...
	m_accumulationProgram->attach(glow::Shader::fromFile(gl::FRAGMENT_SHADER, transparencyShaderFilePath + "wavg_translucent.frag"));
	...
	m_quad = new glowutils::ScreenAlignedQuad(glow::Shader::fromFile(gl::FRAGMENT_SHADER, transparencyShaderFilePath + "wavg_post.frag"));
