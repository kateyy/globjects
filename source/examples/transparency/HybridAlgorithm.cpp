...
void HybridAlgorithm::initialize(const std::string & transparencyShaderFilePath, glow::Shader *vertexShader, glow::Shader *geometryShader) {
	...
	m_opaqueProgram->attach(glow::Shader::fromFile(gl::FRAGMENT_SHADER, transparencyShaderFilePath + "hybrid_opaque.frag"));
	...
	m_depthKTabProgram->attach(glow::Shader::fromFile(gl::FRAGMENT_SHADER, transparencyShaderFilePath + "hybrid_depthktab.frag"));
	if (geometryShader != nullptr) m_depthKTabProgram->attach(geometryShader);

	m_visibilityKTabProgram = new glow::Program;
	m_visibilityKTabProgram->attach(glow::Shader::fromFile(gl::COMPUTE_SHADER, transparencyShaderFilePath + "hybrid_visibilityktab.comp"));

	m_colorProgram = new glow::Program;
    m_colorProgram->attach(vertexShader);
	m_colorProgram->attach(glow::Shader::fromFile(gl::FRAGMENT_SHADER, transparencyShaderFilePath + "hybrid_color.frag"));
	...
	m_compositionQuad = new glowutils::ScreenAlignedQuad(glow::Shader::fromFile(gl::FRAGMENT_SHADER, transparencyShaderFilePath + "hybrid_post.frag"));
